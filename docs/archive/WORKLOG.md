...
- 继续独立核对输入发现单次宽文件列表可产生大量重复长路径，一次100项就击穿32K窗口。改为通用20项分页（next_offset），保留继续取证能力，不加调查总步数或总时限。45个随机文件跨页完整且无重复的本地回归通过。
- 通用工具可探索性补齐：允许沿 manifest/import 线索进入已观察根内的子目录搜索，解决SDK深层目录被固定深度掩盖的问题；不增加任何产品/目录名单。新增 read_evidence 按原证据id、JSON pointer及分页回读，续查索引带脱敏参数，不必为找回已有数据重跑调查。随机目录与证据分页本地回归通过。
- 二进制检索增加按需 context_bytes（最多2048），窗口越大每页命中数自动减少，提供足以理解真实loader代码的上下文而非只给短词片段。仍为分块文件读，不映射/执行目标。
- 执行者 learned_presentation 经顾问修补目标绑定和完整局限标记后已接入dashboard代码；只接受通用生成计划安装来源及同实例验收事件，不改变原未接入状态。尚待独立真实事件后部署，未把代码就绪称为Hook成功。
...
## 2026-09-10 advisor real positive + compatible new instance (23:15 local)
- New actual Goose proposal completed via configured buffered response transport. Its corrected candidate supplied the full Hook source. Generic container normalization records a JSON-encoded object missing only its terminal brace; source text is not edited. file_plan without a top-level plan is now rejected, rather than persisted as executable success.
- Source run pid_44187_1789052847065; generic installer approved isolated workspace and candidate digest. Actual source SHA256 b8f2a8d2b627b66689ea4402f52c9af83338c0bd758141f68676ec527f47649f matches installed bytes.
- Real first target emitted hook.loaded and one verified read before/after pair, 0 invalid. Own test driver needed syntax and isolated provider config corrections; no Hook source edits. Fresh own CLI PID77504 reuses exact build/argv/cwd and same plan without new Goose calls; independent fresh nonce/PID/create_time check observed one real read pair, 0 invalid.
- Real reuse exposed stale duplicate fingerprint shadowing: classify now selects latest compatible validation; identical exact-build observations without an explicit evolution id become revisions. No product-name branch.
- Bound verification imported into isolated 8081 experience store; actual source remains attributed to old instance. Page adds explicit reuse label and verified execution snapshot, not a continuous-health or blocking claim. No global user application restart, no 8080 operation, no production DB mutation.
- Scope and reproduction recorded in docs/stage1/MVP_DEMO.md. This is supervised learned-code success, not autonomous blind coverage. Assets and unattended orchestration remain incomplete.
- Focused plus existing discovery/matcher regression command: 73 tests OK. Real runtime evidence above, not this count, is the acceptance basis.
