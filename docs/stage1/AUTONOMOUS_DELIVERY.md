...
2026-09-11 用户要求改为通用自动流程后，本机部署安装范围已扩到当前用户目录 `/Users/mac`。目标仍由进程扫描发现，安装子目录由 Goose 从目标证据调查得出，执行器校验文件摘要、备份和范围。不按产品名设置安装目录或选取适配器，不重启用户应用。点击首页“扫描分析”，结果会自动刷新。
...
## 通用扫描流程更新（2026-09-11 15:22）

修复通用文件计划错误使用进程 cwd 的问题，保留同实例候选的原调查来源。自动流程拒绝产品专用适配器；精确指纹与候选检查点不再单独结束流程，执行失败可续查并向 Goose 提供上次执行反馈。并发扫描共用单次执行锁，界面显示扫描进度。

本次实际运行已在调查所得目录自动安装 Codex、ZCode 两份候选，文件字节与 Goose 输出一致，见 `artifacts/autonomous-service/generic-install-verification.json`。这两项目前均等待加载与真实工具事件，不能算完整活动验收。上文“未安装”是此前演示范围限制下的历史结果。未知 Agent 的整体成功率尚未验证；不承诺任意运行时都能热加载。305 项回归通过（4 项跳过），另有扫描界面与目录/续查专项检查。

## 页面配置与标准资料（2026-09-11）

展开“Goose 模型配置”可修改 OpenAI 兼容 API 地址、模型和密钥。点击“验证并保存”会运行独立的无扩展 Goose 调用，检查 assistant 消息是否包含随机验证串，成功才原子保存并用于后续调查。失败保留原配置；进行中的调查继续使用启动时的配置。同地址可留空沿用密钥，更换地址必须填写新密钥。密钥不回显，设置保存在运行目录权限为 0600 的 model_settings.json。

新调查的 identity/asset 都要求 display.version=1、summary、facts（label/value 文本列表）、scope；未知项与证据引用仍独立保留，中文描述要求由 Goose 执行。展示字段经过结构校验，不能代替证据校验。历史资料不改写事实，使用字段列表呈现，原始 JSON 放在二级展开项。

非 Agent 统一放到“其他类别（非 Agent）”，简洁显示调查名称、类别、进程、版本、用途及依据与调查活动；分类仍取自 Goose 的 roles，没有产品专属判断分支。

真实界面模型验证成功并保存：Goose 返回 ASG-dcb21ded57eae4ca。309 项全量运行出现两个生命周期短超时问题，随后包括这两项在内的 11 项专项复跑通过；没有声称全量首次通过。
