...
## 追加：SOC 网关日志整改（2026-09-21，soc-open-api 70747f7）

发现：网关 stdout 把每个请求正文（含 Hook 事件对话内容）与心跳正文（含 Agent
API key）全量回显，本地日志 40 分钟内涨至 104MB，且违反"日志不得输出凭据正文"
要求；正文回显同时污染运维日志检索（G13 点名问题）。

整改（运行二进制 sha256 前缀 c7696c04，备份 .bak-20260921c，旧日志已归档）：
- 请求/响应正文回显默认关闭（SOC_GATEWAY_LOG_BODIES=1 可临时恢复），保留方法/
  路径/长度等结构化字段；数据库 call log 不受影响。
- 心跳日志改为只记 application/agent 标识，不再打印含 token 的正文。

验收：中间件与控制层 Go 测试通过；新网关 health 200、带鉴权心跳 200、
新日志文件中 token 字符串 0 次命中。

## 追加：0.9.5 包（含交付说明）全生命周期复验（2026-09-21，提交 a72db73）

0.9.5 相对 0.9.4 仅新增包内 README 的"构建绑定 / 已支持范围 / 已知限制"三段
