...
回归：66 个测试模块，Ran 438，FAILED (errors=1)。
...
统一规则（任务书第一节）：身份字段完整率 1.0、证据关联实例 1.0、跨实例错误关联 0、合成事件混入 0、展示重复数 0（Hook 日志重复序号 + 控制回执重复 request_id）；逐条报告见 `artifacts/acceptance/test-reports.json`（84 行）。
...
| 2 完整采集真实输入输出和工作流 | 工具名称/参数/结果覆盖率 100% |  | **通过** | 43 对，带参数/结果 43 对 |
...
| 2 完整采集真实输入输出和工作流 | 正常用户实例独立验收 |  | **失败** | 当前正常会话独立对账：pid=9601、通道=api、3 个完整回合、9 条文本、0 条不一致、核查时间=2026-09-20T03:05:40.106617+00:00；要求至少 10 回合，逐回合明细见 artifacts/acceptance/normal-conversation-9601.json。隔离实例或共享引擎驱动均不能替代该实例口径。 |
...
| 5 发现程序退出后独立运行 | 闭包式安装可独立启动并服务（不依赖发现源码树） |  | **通过** | portable-install.json：仅复制运行闭包到新目录，服务启动并服务 47 个已绑定实例；发现模块文件 [] |
...
| 6 WorkBuddy 与替代接入路径 | 候选发现 30 秒内 |  | **通过** | 手动扫描后可见延迟 0.2 秒 |
| 6 WorkBuddy 与替代接入路径 | 调查可见性（状态/原因/首条活动） |  | **通过** | activity 接口返回状态与首条活动 |
| 6 WorkBuddy 与替代接入路径 | 三目标状态与能力覆盖记录 |  | **通过** | multi-target.json（目标：ZCode、Codex、OpenCodex） |
...
| Codex (PID 9601) | 5/10 | proven | proven | unsupported | not_reached |
| OpenCodex (PID 97143) | 3/10 | not_reached | not_reached | not_reached | not_reached |
