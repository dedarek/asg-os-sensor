...
def supports_direct_events(bundle):
    ...
    for item in files:
        ...
        learned_control_client = (client_calls and 'tools/pre-execute' in content
                                  ...
                                  and 'hook_control_client' in content and denied_ack)
...
def build(bundle):
    ...
    files={'transport.json':canonical({'transport':'soc-direct-v1','control':'before_tool_call','events':'user_model_tool_session','requires_asg_service':False,'installer_revision':installer_revision()}), 'soc_client.mjs':(Path(__file__).parent/'package_runtime/soc_client.mjs').read_bytes(),'soc_client.py':(Path(__file__).parent/'package_runtime/soc_client.py').read_bytes(),'recipe-bundle.json':canonical(bundle),'install.py':(Path(__file__).parent/'package_runtime/install.py').read_bytes(),
...
def installer_revision():
    paths=[Path(__file__),Path(__file__).parent/'package_runtime/install.py',Path(__file__).parent/'package_runtime/soc_client.mjs',Path(__file__).parent/'package_runtime/soc_client.py']+[ROOT/'runtime'/name for name in ('learned_install.py','file_lock.py','recipe_bundle.py')]
